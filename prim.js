var HeapSorter = {
  ...
  heapify: function (A, i) {
    ...
    if (r < A.length) {
      if (l <= this.heap_size && A[l][2] < A[i][2]) smallest = l;
      else smallest = i;
      if (r <= this.heap_size && A[r][2] < A[smallest][2]) smallest = r;
    }
    if (smallest != i) {
      var temp = A[smallest];
      A[smallest] = A[i];
...
var Prim = function (_grafo, _callback) {
  ...
  for (var key in grafo.arestas) {
    if (!indiceArestas[grafo.arestas[key][0]]) indiceArestas[grafo.arestas[key][0]] = [];
